...
`go-ruby-omniauth/omniauth` reimplements Ruby's `omniauth` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-omniauth/omniauth` directly, with no Ruby runtime;
...
Because `omniauth` is pure Go with cgo disabled, it also compiles to **WebAssembly**
